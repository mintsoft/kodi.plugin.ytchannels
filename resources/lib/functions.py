...
def init_database():
    with db:
        ...
        cur.execute(
            "create table if not exists Channels (Folder TEXT, Channel TEXT, Channel_ID TEXT,thumb TEXT,sort INT)")
        db.commit()
        cur.close()
    return
...
def get_channels(foldername):
    ...
    cur.execute("SELECT Channel,Channel_ID,thumb,sort,ROWID FROM Channels WHERE Folder=? ORDER BY sort ASC",
                (foldername,))
...
def add_channel(foldername, channel_name, channel_id, thumb):
    ...
    cur.execute("begin")
    try:
        cur.execute("DELETE FROM Channels WHERE Folder = ? AND Channel = ? AND Channel_ID = ? AND thumb = ?",
                    (foldername, channel_name, channel_id, thumb))
    except:
        pass
...
def search_channel(channel_name):
    ...
    req_url = 'https://www.googleapis.com/youtube/v3/search?q=%s&type=channel&part=snippet&maxResults=%s&key=%s' % (
    channel_name, str(result_num), YOUTUBE_API_KEY)
    ...
    for x in range(0, len(decoded_data['items'])):
        ...
        req1 = 'https://www.googleapis.com/youtube/v3/channels?part=contentDetails&id=%s&key=%s' % (
...
def search_channel_by_username(username):
    req_url = 'https://www.googleapis.com/youtube/v3/channels?part=contentDetails&forUsername=%s&key=%s' % (
    username, YOUTUBE_API_KEY)
    read = read_url(req_url)
    decoded_data = json.loads(read)
    listout = []
    ...
    if decoded_data['pageInfo']['totalResults'] == 1:
        ...
        req2 = 'https://www.googleapis.com/youtube/v3/channels?part=snippet&id=%s&key=%s' % (
        channel_id, YOUTUBE_API_KEY)
        read = read_url(req2)
        decoded_data = json.loads(read)
...
def get_channel_id(channel_username):
    req_url = 'https://www.googleapis.com/youtube/v3/channels?part=contentDetails&forUsername=%s&key=%s' % (
    channel_username, YOUTUBE_API_KEY)
    read = read_url(req_url)
    decoded_data = json.loads(read)
...
def get_latest_from_channel(channel_id, page, filter_shorts):
    ...
    video_req_url = 'https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id=%s&key=%s' % (
    ','.join(videoids), YOUTUBE_API_KEY)
...
def get_playlists(channelID, page):
    if page == '1':
        url = 'https://www.googleapis.com/youtube/v3/playlists?part=snippet&channelId=%s&maxResults=10&key=%s' % (
        channelID, YOUTUBE_API_KEY)
    else:
        url = 'https://www.googleapis.com/youtube/v3/playlists?part=snippet&channelId=%s&maxResults=10&pageToken=%s&key=%s' % (
        channelID, page, YOUTUBE_API_KEY)
    read = read_url(url)
    decoded_data = json.loads(read)
    playlists = []
    try:
...
def get_livestreams(channelID, page):
    if page == '1':
        url = 'https://youtube.googleapis.com/youtube/v3/search?part=snippet&channelId=%s&eventType=live&order=date&type=video&key=%s' % (
        channelID, YOUTUBE_API_KEY)
    else:
        url = 'https://youtube.googleapis.com/youtube/v3/search?part=snippet&channelId=%s&eventType=live%order=date%type=video&pageToken=%s&key=%s' % (
        channelID, page, YOUTUBE_API_KEY)
    read = read_url(url)
    decoded_data = json.loads(read)
    livestreams = []
